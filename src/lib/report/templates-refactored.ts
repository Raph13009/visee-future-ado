...
import { TRAITS } from "@/pages/TestPersonnalite";
import {
  getTraitDescription,
  getTraitStrengths,
  getTraitGrowthAreas,
...
export function buildReportModel(scores: Record<string, number>): ReportModel {
  ...
  while (topStrengths.length < 6 && sortedTraits.length > topStrengths.length / 2) {
    const trait = sortedTraits[Math.floor(topStrengths.length / 2)];
    const strengths = getTraitStrengths(trait.name, trait.score);
  ...
  while (areasForGrowth.length < 6 && sortedTraits.length > 0) {
    const trait = sortedTraits[Math.max(0, sortedTraits.length - Math.ceil(areasForGrowth.length / 2))];
    const areas = getTraitGrowthAreas(trait.name, trait.score);
    if (areas.length > 0) {
  ...
  const lowestTraits = sortedTraits.slice(-2).map(t => t.name);
